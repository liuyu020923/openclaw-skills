...
if $PYTHON "$SCRIPT" --auth-only 2>/dev/null | $PYTHON -c "import sys,json; d=json.load(sys.stdin); assert d['success']" 2>/dev/null; then
    ok "auth-only"
else
...
if $PYTHON "$SCRIPT" --irn "000298" --ds AL --num 1 2>/dev/null | $PYTHON -c "import sys,json; d=json.load(sys.stdin); assert d['success']; assert d['total']>=0" 2>/dev/null; then
    ok "wipo-irn-search"
else
...
if $PYTHON "$SCRIPT" --prod "chair" --ds US --num 1 2>/dev/null | $PYTHON -c "import sys,json; d=json.load(sys.stdin); assert d['success']" 2>/dev/null; then
